...
db = db.getSiblingDB('pet_platform_chat');
...
db.createCollection('messages', {
  validator: {
    $jsonSchema: {
      bsonType: 'object',
      required: ['chat_room_id', 'sender_id', 'message_type', 'created_at'],
      properties: {
        chat_room_id: {
          bsonType: 'string',
        ...
        message_type: {
          enum: ['text', 'image', 'video', 'audio', 'file', 'location', 'pet_info', 'system'],
        ...
        flagged: {
          bsonType: 'bool',
          description: 'Whether message has been flagged for moderation'
        },
        flag_reason: {
          bsonType: 'string',
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        action: {
          bsonType: 'string',
          description: 'Action performed (e.g., "view_pet", "swipe_like", "send_message")'
        },
        entity_type: {
          bsonType: 'string',
          description: 'Type of entity acted upon (e.g., "pet", "user", "match")'
        },
        entity_id: {
          bsonType: 'string',
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        event_type: {
          enum: ['suspicious_listing', 'multiple_accounts', 'rapid_swipes', 'fake_profile', 'payment_fraud', 'spam_behavior'],
        ...
        action_taken: {
          enum: ['none', 'flagged', 'suspended', 'blocked', 'require_verification'],
